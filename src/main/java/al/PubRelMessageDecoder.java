...
@CacheScoped
public class PubRelMessageDecoder
        extends AbstractMessageDecoder<PubRel> {
    private static final Logger LOGGER = LoggerFactory.getLogger(PubRelMessageDecoder.class);
    private final CachedMessages cachedMessages = CachedMessages.INSTANCE;

    public PubRel decode(Channel channel, ByteBuf remainingByteBuf, byte fixedHeader) {
        if (ProtocolVersion.MQTTv3_1_1 == channel.attr(AttributeKeys.MQTT_VERSION).get() &&
                !validReservedFlagsBits(fixedHeader)) {
            if (LOGGER.isDebugEnabled()) {
                LOGGER.debug("A client (IP: {}) sent a Pubrel with an invalid fixed header. Disconnecting client.",
        ...
        if (remainingByteBuf.readableBytes() < 2) {
            if (LOGGER.isDebugEnabled()) {
                LOGGER.debug("A client (IP: {}) sent a Pubrel without a message id. Disconnecting client.",
                        ChannelUtils.remoteIP(channel).orElse("UNKNOWN"));
            }
            channel.close();
            remainingByteBuf.clear();
