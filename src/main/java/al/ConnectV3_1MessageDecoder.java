...
public class ConnectV3_1MessageDecoder extends AbstractMessageDecoder<Connect> {
    ...
    private static final int VARIABLE_HEADER_LENGTH = 12;
    ...
    public Connect decode(Channel channel, ByteBuf remainingByteBuf, byte fixedHeader) {
        ...
        if (!validProtocolVersion(variableHeader)) {
            if (LOGGER.isDebugEnabled()) {
                LOGGER.debug("A client (IP: {}) connected with an invalid protocol name. Disconnecting client.",
                        ChannelUtils.remoteIP(channel).orElse("UNKNOWN"));
            }
            channel.close();
            return null;
        ...
        connect.setProtocolVersion(ProtocolVersion.MQTTv3_1);
