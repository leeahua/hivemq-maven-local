...
public class PollTransactionalComputable
        extends AbstractQueuedMessagesLookupTable
        implements TransactionalComputable<ClientSessionQueueEntry> {
    private static final Logger LOGGER = LoggerFactory.getLogger(PollTransactionalComputable.class);
    private final String clientId;
    private final Long timestamp;
    private final Store store;
    private final QueuedMessagesSerializer serializer;

    public PollTransactionalComputable(
    ...
    public ClientSessionQueueEntry compute(@org.jetbrains.annotations.NotNull Transaction txn) {
        ...
        try {
            ByteIterable searchBothRange = cursor.getSearchBothRange(searchKey, XodusUtils.toByteIterable(this.serializer.serializeTimestamp(this.timestamp)));
            if (searchBothRange != null) {
                do {
                    if (cursor.getKey().compareTo(searchKey) == 0) {
                        ...
                        removeLookupTable(this.clientId, this.timestamp);
