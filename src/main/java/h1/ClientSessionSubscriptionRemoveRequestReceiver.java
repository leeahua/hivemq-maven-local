...
@CacheScoped
public class ClientSessionSubscriptionRemoveRequestReceiver
        extends GenericRequestReceiver
        implements ClusterReceiver<ClientSessionSubscriptionRemoveRequest> {
    private static final Logger LOGGER = LoggerFactory.getLogger(ClientSessionSubscriptionRemoveRequestReceiver.class);
    private final ClientSessionSubscriptionsSinglePersistence clientSessionSubscriptionsSinglePersistence;

    @Inject
    ClientSessionSubscriptionRemoveRequestReceiver(
    ...
    public void received(@NotNull ClientSessionSubscriptionRemoveRequest request, @NotNull ClusterResponse response, @NotNull String sender) {
        Preconditions.checkNotNull(request, "Request must not be null");
        Preconditions.checkNotNull(response, "Response must not be null");
        Preconditions.checkNotNull(sender, "Sender must not be null");
        LOGGER.trace("Received client session subscription REMOVE request for client {}", request.getClientId());
        if (!responsible(request.getKey(), response, Void.class)) {
            return;
        }
        ListenableFuture<Void> future = this.clientSessionSubscriptionsSinglePersistence
                .removeSubscription(request.getClientId(), request.getTopic());
        sendResult(future, response);
    }

    protected void onFailure(Throwable t) {
        LOGGER.error("Exception while processing subscription remove request.", t);
    }
}
